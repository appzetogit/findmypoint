  ...
  useEffect(() => {
    try {
      ...
      if (saved) {
        ...
        const defaultNotifs = [
          { id: 1, title: "Welcome to FindmyPoint!", text: "Your search for local points made simple. Complete your profile details to unlock premium configurations.", time: "2 hours ago", type: "info", read: false },
    ...
    <div className="space-y-6">
      <div className="flex justify-between items-center">
        <div>
          ...
          <p className="text-xs text-muted-foreground mt-0.5">Security updates and confirmation summaries.</p>
        </div>
          ...
          {notifications.map(notif => (
            ...
            >
              <div className="flex items-start gap-3">
                <span className={`flex h-2.5 w-2.5 mt-1.5 rounded-full shrink-0 ${
                  notif.type === 'success' 
                    ? 'bg-emerald-500' 
                    : notif.type === 'warning' 
                      ? 'bg-amber-500' 
                ...
                <div>
                  ...
                  <p className="text-[11px] text-muted-foreground mt-1 leading-relaxed pr-8">{notif.text}</p>
                  <span className="text-[9px] text-slate-400 font-bold block mt-2">{notif.time}</span>
                </div>
              </div>
              <button 
